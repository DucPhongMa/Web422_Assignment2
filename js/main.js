...
function clickRowToShowModal(data){
    document.querySelectorAll('#moviesTable tbody tr').forEach((row) => {
        row.addEventListener('click', (e) => {
            ...
            fetch(`https://tough-erin-sheath-dress.cyclic.app/api/movies/${clickedId}`)
            .then((res) => res.json())
            .then((data) => {
                console.log(data)
                let commentsList = "";
...
function loadMovieData(title = null) {
    let url = title
    ? `https://tough-erin-sheath-dress.cyclic.app/api/movies?page=${page}&perPage=${perPage}&title=${title}`
    : `https://tough-erin-sheath-dress.cyclic.app/api/movies?page=${page}&perPage=${perPage}`;
